...
import { tipLink, tipsSummary, tipMethodNote } from '@dastarhan/shared/staff/tips.logic';
import type { TipRecord, TipMethod } from '@dastarhan/shared/staff/tips.logic';
...
// ═══════════════ СОТРУДНИКИ И РОЛИ: ТЕКСТЫ ═══════════════

export const STAFF_COPY = {
  // Владелец должен понимать границу доступа раньше, чем заведёт людей
  accessNote: 'Кассир видит только кассу. Отчёты и себестоимость — только у владельца и менеджера.',

  card: {
    since: (date: string, lastSeen: string) =>
      `В команде с ${date} · последний вход ${lastSeen}`,
    fullName: 'ФИО',
    resetPin: 'Сбросить',
    // Роль привязана к точке, а не к человеку: в сети заведений
    // один и тот же сотрудник бывает менеджером и кассиром одновременно
    multiRole: 'на одной точке менеджер, на другой кассир — так можно',
    earned: (month: string) => `Начислено в ${month}`,
    included: 'включено',
    block: (name: string) => `Заблокировать ${name}`,
    fullLog: 'Весь журнал действий →',
  },

  tips: {
    title: (month: string) => `Чаевых в ${month}`,
    // Чаевые — деньги официанта, не заведения. Брать с них процент
    // или показывать в выручке значит обманывать и сотрудника,
    // и владельца в отчёте о прибыли
    policy: 'Мы не берём процент с чаевых и не показываем их в выручке заведения.',
  },

  roles: {
    preset: 'Готовая роль',
    fourStates: 'Четыре состояния права',
