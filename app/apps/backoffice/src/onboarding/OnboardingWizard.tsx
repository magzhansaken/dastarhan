...
// ═══════════════ ТЕКСТЫ МАСТЕРА НАСТРОЙКИ ═══════════════
// Обещание с сайта — первый чек за 15 минут. Значит мастер не может
// требовать того, что владелец сделает потом: карту зала, брони,
// доставку. Настраиваем минимум для продажи, остальное предлагаем позже.
...
export const ONB_COPY = {
  ...
  location: {
    ...
    // БИН подтягивает реквизиты: меньше полей руками — меньше ошибок
    binOk: 'подтверждён — данные ИП подтянулись автоматически, вводить вручную не нужно.',
  },

  // Шаг «Касса» — две развилки вместо одной формы
  fiscal: {
    // Заведение может уже иметь кассу в Webkassa. Требовать новую —
    // значит заставить платить дважды и путать чеки в ОФД
    existing: 'Уже есть касса в Webkassa — подключим по логину, чеки пойдут в ту же кассу.',
    fresh: 'Оформим новую фискальную кассу — обычно активируется в тот же день.',
    login: 'Логин кабинета',
    checkOk: 'Проверка связи прошла: касса найдена, ОФД отвечает.',
  },

  // Шаг «Меню» — пять блюд, а не всё меню
  menu: {
    // Полное меню на старте — час работы и повод бросить настройку.
    // Пять блюд достаточно, чтобы пробить первый чек сегодня
    starter: 'Пять блюд для старта — остальное добавите вечером.',
    more: (n: number) => `и ещё ${n} блюдо ·`,
    showAll: 'посмотреть все',
  },

  // Шаг «Люди»
  staff: {
    roleCashier: 'Кассир, официант',
    roleCook: 'Повар · KDS',
    // Владелец должен понимать границу доступа до того,
    // как заведёт сотрудников
    privacy: 'Кассир видит только кассу. Отчёты и себестоимость — только у вас.',
  },

  // Шаг «Оплата»
  payment: {
    connected: 'Подключено',
    connectedNote: '· оплата появится на кассе сразу.',
    connect: 'Подключить',
    later: 'Пригодится позже — можно включить в один клик',
  },

  // Финал
  finish: {
    oneLeft: 'Осталось одно действие',
    oneLeftNote: ', внесите размен — и можно пробивать первый чек.',
    sendLink: 'Ссылка на планшет по SMS',
    readyTitle: 'Настроено',
    readyList: 'Точка, Webkassa, 64 блюда, 3 сотрудника, Kaspi QR.',
    laterTitle: 'Можно потом',
    laterList: 'Карта зала, брони, доставка, QR-меню на столы.',
    whatReady: 'Что уже готово',
    support: 'Асель, поддержка',
    supportNote: 'ответит за 3 минуты · Рус/Қаз',
  },
} as const;

/** Склонение блюд для строки «и ещё N блюд». */
export function dishesMore(n: number): string {
