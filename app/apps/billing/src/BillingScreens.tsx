...
// ═══════════════ БИЛЛИНГ КЛИЕНТА: ТЕКСТЫ ═══════════════
// Владелец должен понимать, за что платит и что будет, если
// добавить точку или сменить тариф — до нажатия, а не после.

export const BILLING_COPY = {
  // Пересчёт со дня изменения, а не с начала месяца:
  // добавил точку 20-го — платишь за 10 дней, а не за месяц
  proration: 'Добавили точку — цена пересчитается с этого дня. Убрали — тоже.',
  allDocs: 'Все документы →',

  planChange: {
    // Переход действует со следующего месяца: клиент не теряет
    // оплаченные функции посреди периода
    note: 'переход действует со следующего месяца',
    downgrade: 'Понизить тариф',
    toNetwork: 'Перейти на Сеть',
    current: (plan: string, points: string) => `Тариф ${plan} · ${points}`,
  },

  // Что теряется и что появляется — прямым текстом, без «и другие функции»
  planContents: {
    START: 'Без доставки, лояльности и ИИ-помощника. Отчёты — базовые.',
    BUSINESS: 'Доставка, лояльность, ИИ-помощник, все отчёты и брони.',
    NETWORK: 'Центральный склад, перемещения между точками, франшиза и роялти.',
  } as Record<string, string>,

  payTo: (entity: string) =>
    `Оплата уходит на ${entity}. Закрывающие документы придут на почту сразу.`,
} as const;

/** Пересчёт при изменении числа точек в середине месяца. */
export function prorate(
  pricePerLocation: number, daysLeft: number, daysInMonth: number, delta: number,
): number {
  if (daysInMonth <= 0) return 0;
  return Math.round(pricePerLocation * delta * daysLeft / daysInMonth);
}
