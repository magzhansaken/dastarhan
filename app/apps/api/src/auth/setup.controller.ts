...
export class SetupController {
  ...
  async readiness(@Req() req: any) {
    ...
    const [products, cards, staff, terminal, warehouse, stock] = await Promise.all([
      ...
      // У Warehouse нет связи location — только locationId,
      // поэтому сначала берём точки аккаунта
      this.prisma.location.findMany({ where: { accountId: acc }, select: { id: true } })
        .then((locs) => this.prisma.warehouse.findFirst({
