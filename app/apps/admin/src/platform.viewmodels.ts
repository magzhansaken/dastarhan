...
// ═══════════════ КАРТОЧКА КЛИЕНТА: ПОДПИСКА ═══════════════

export const SUBSCRIPTION_PANEL = {
  historyTitle: 'История подписки',
  // Сводка одной строкой вместо таблицы: менеджеру важно понять
  // репутацию клиента за секунду, а не изучать историю платежей
  historySummary: (renewals: number, graces: number, blocks: number) =>
    `${renewals} продлений, ${graces === 0 ? 'ни одной отсрочки' : graces + ' отсрочка'}, ` +
    `${blocks === 0 ? 'ни одной блокировки' : blocks + ' блокировка'}`,
  actionsTitle: 'Действия с подпиской',
  extend: (sum: string) => `Продлить месяц · ${sum}`,
  grace: 'Дать отсрочку 7 дней',
  freeze: 'Заморозить аккаунт',
  // Автосписания нет намеренно: доверие важнее удобства сбора денег.
  // Клиент подтверждает каждый платёж и не боится «незаметных» списаний
  payNote: 'Средний срок оплаты счёта — 1,8 дня. Автосписания нет: клиент подтверждает каждый платёж.',
} as const;

export const HEALTH_PANEL = {
  title: 'Здоровье клиента',
  allGood: 'Кассы в сети, чеки идут, выручка растёт',
  staffLabel: 'Сотрудник клиента',
} as const;

// ═══════════════ ЖУРНАЛ ДЕЙСТВИЙ ═══════════════
// Два раздельных лога: что делали мы и что делал клиент.
// Смешивать нельзя — при разборе спора важно, кто именно что нажал.

export const AUDIT_LOG = {
  ourTitle: 'Что мы делали с аккаунтом',
  ourNote: 'видно и нам, и клиенту',
  clientTitle: 'Что делал клиент',
  // Содержимое чеков не показываем: это коммерческая тайна заведения,
  // и вендор не должен иметь к ней доступ даже технически
  privacy: 'Лог хранится 24 месяца. Мы не показываем в нём содержимое чеков — только действия.',
} as const;

// ═══════════════ ПОДДЕРЖКА: ОТВЕТ КЛИЕНТУ ═══════════════

export const TICKET_REPLY = {
  header: (client: string, channel: string, opened: string) =>
    `${client} · ${channel} · открыт ${opened}`,
  placeholder: (channel: string) => `Ответить клиенту в ${channel}…`,
...
export const PLAN_FEATURE_MATRIX = {
  title: 'Матрица тариф × функция',
  // Изменение применяется к новым платежам, а не сразу:
  // клиент не должен потерять функцию посреди оплаченного месяца
