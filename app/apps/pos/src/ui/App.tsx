...
import { PinScreen, OrderScreen, PaymentScreen } from './screens/PosScreens';
import { OrderState, OrderEvent, reduceOrder, orderTotals } from '../offline/orderReducer';
import { CatalogItem } from './viewmodels';
    ...
    <div className="hall-screen">
      <header className="hall-head">
        <div className="ch-cashier">
          <h2>Зал</h2>
          <span className="label-mono">На столах сейчас {fmt(props.tables.reduce((s, t) => s + (t.total ?? 0), 0))}</span>
          <span className="label-mono">Открытых заказов {props.tables.filter((t) => t.state !== 'free').length}</span>
        </div>
        <button className="btn btn-accent" onClick={props.onQuickOrder}>Новый заказ</button>
      </header>
